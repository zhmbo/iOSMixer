...
void deleteComments(NSString *directory) {
    ...
    for (NSString *fileName in files) {
        ...
        // 过滤删除注释文件
        if ([fileName isEqualToString:@"WanYuSDK.h"]||[fileName isEqualToString:@"SDWebImageDownloader"]) {
            continue;
        }
...
//                continue;
        ...
        {
            if ([fileName.lowercaseString hasPrefix:oldName.lowercaseString]) {
                newClassName = [newName.lowercaseString stringByAppendingString:[fileName substringFromIndex:oldName.length]];
            } else {
                //            newClassName = [newName stringByAppendingString:fileName];
                //不包含前缀的不加新前缀
                newClassName = fileName;
        ...
        if ([fileExtension isEqualToString:@"h"]) {
            ...
            if ([files containsObject:mFileName]){
                ...
                if([fileContent containsString:oldName]){
                    [fileContent replaceOccurrencesOfString:oldName withString:newName options:NSLiteralSearch range:NSMakeRange(0, fileContent.length)];
                    [fileContent writeToFile:mFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
                }
                NSMutableString *hfileContent = [NSMutableString stringWithContentsOfFile:hFilePath encoding:NSUTF8StringEncoding error:nil];
                if([hfileContent containsString:oldName]){
                    [hfileContent replaceOccurrencesOfString:oldName withString:newName options:NSLiteralSearch range:NSMakeRange(0, hfileContent.length)];
                    [hfileContent writeToFile:hFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
                }
            }
                ...
                NSMutableString *hfileConten = [NSMutableString stringWithContentsOfFile:hFilePath encoding:NSUTF8StringEncoding error:nil];
                if([hfileConten containsString:oldName]){
                    [hfileConten replaceOccurrencesOfString:oldName withString:newName options:NSLiteralSearch range:NSMakeRange(0, hfileConten.length)];
                    [hfileConten writeToFile:hFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
                }
            }
        }else if ([fileExtension isEqualToString:@"json"]) {
            NSString *jsonFilePath = [[sourceCodeDir stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:@"json"];
            NSError *error = nil;
            NSMutableString *fileContent = [NSMutableString stringWithContentsOfFile:jsonFilePath encoding:NSUTF8StringEncoding error:&error];
            if([fileContent containsString:oldName]){
                [fileContent replaceOccurrencesOfString:oldName withString:newName options:NSLiteralSearch range:NSMakeRange(0, fileContent.length)];
                [fileContent writeToFile:jsonFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
            }
        }
    }
}
